	...
	def ai_insert(self): #AI
		...
		for x in range(emn): #if there is no victory, make possible that computer can not lose
			...
			if self.victory(1): #if the player can win
				self.actualgame = backup[:]
				return empty[x] #return his index
		self.actualgame = backup[:]
		return self.random_set() #if there is no possibilities, return a random index
	def no_insert(self):
...
class Setup:
	def __init__(self):
		...
		self.start = False
	def ok(self):
		self.results = [self.infoboxes[0][1].get(), self.infoboxes[1][1].get(), bool(self.aivar.get())]
		if "" in self.results:
